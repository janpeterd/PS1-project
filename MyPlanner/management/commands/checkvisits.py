...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        unassigned_visits = GroupVisit.objects.filter(
            toegewezenGids__id=1, tijdstipAanvraag__lt=five_days_ago, gearchiveerd=False
        )
        self.stdout.write(
            f"{len(unassigned_visits)} niet toegewezen bezoeken gevonden."
        )
        ...
        if len(unassigned_visits) != 0:
            for visit in unassigned_visits:
                ...
                mail_subject = (
                    "Groep "
                    + str(chosen_visit.id)
                    + " is geannuleerd, kies een nieuw tijdstip"
                )
                msg_html = render_to_string(
                    "MyPlanner/mail/archived.html",
                    {"chosen_visit": chosen_visit, "domain": domain},
                )
                msg_plain = render_to_string(
                    "MyPlanner/mail/archived.txt",
                    {"chosen_visit": chosen_visit, "domain": domain},
                )
        ...
        past_visits = GroupVisit.objects.filter(
            datum__lt=timezone.now(), gearchiveerd=False
        )
        self.stdout.write(
            f"{len(past_visits)} bezoeken gevonden die al voorbij zijn, maar nog niet gearchiveerd zijn."
        )
